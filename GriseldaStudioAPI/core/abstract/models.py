...
# De aqui heledaran los modelos de la aplicacion
class AbstractModel(models.Model):  
    ...
    public_id = models.UUIDField( # db_index le creo el indice en la BD
        db_index=True, unique=True, default=uuid.uuid4, editable=False
    )
    created = models.DateTimeField(auto_now_add=True) # se crea automaticamente cuando se crea el registro
    updated = models.DateTimeField(auto_now=True) # se actualiza automaticamente cuando se modifica el registro
    is_deleted = models.BooleanField( # campo para marcar un registro como eliminado
        default=False
    )  # para no eliminar los registros de la base de datos, sino marcarlos como eliminados

    objects = AbstractManager() # se asigna el administrador de modelos AbstractManager a la propiedad objects
